...
module.exports = (function() {
  ...
  var validateChargeValidity = function(user, body, cart) {
    ...
    try {
      console.log('using credit: ' + acctCredit);
      console.log('cost: ' + cart.cost);
      console.log('balance: ' + cart.balance);
      console.log(body.useAchAccount);
      console.log(body.creditCardToken);
      ...
      } else if (body.useAchAccount && user.achAccount.verified === false) {
        err.push({
          'code': 402,
  ...
  var processCharge = function(user, body, cart, cb){
    ...
    if (err) {
      ...
      try {
        var chargeAmt = cart.cost - body.amtAccountCredit;
        if (body.useAchAccount) {
          var bankAcct = balanced.get('/bank_account/' + user.profile.achAccount.id);
