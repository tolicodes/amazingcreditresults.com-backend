...
exports.throwError = function(res, error){
  ...
  var errors = exports.parseError(error);
...
exports.createError = function(msg, field, code) {
  var err = {
    'code' : code || 400,
    'message': msg
  };
...
exports.frmDt = function (today) {
  return moment.utc(today).format('dddd, MMMM D, YYYY h:mm A') + ' GMT';
};

exports.createPatch = function(changes, nameFields, fieldMap) {
  var patch = {};
  if (changes.name) {
    _(changes.name).forEach(function (val, prop) {
      var key = ((prop === 'title' || prop === 'suffix') ? 'profile.' : 'name.') + prop;
  ...
  _(fieldMap).forEach(function (fullKey, shortKey) {
    if (changes.hasOwnProperty(shortKey)) {
      patch[fullKey] = changes[shortKey];
    }
  });
